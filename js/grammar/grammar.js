...
// 理由：
  //(20 + 2)→数字の計算
  //('20' + 2)→'20'が文字列表記であり、+が文字？連結の記号であるから202が出力される。
...
// 理由：
  //70行目：num1に8が代入される
  //71行目：num2に8が代入された後に、num1がインクリメントされ1つ値が増える
  //75行目：num3に8が代入される
  //76行目：num3が先にインクリメントされ1つ値が増えた状態で、num4に代入される
...
// 理由：「==」2つのオペランドが同じ型でないならばオペランドを変換して、それから厳密な比較を行う
//「===」型変換なしでオペランド同士が厳密に等しければtrueを返す
